...
@router.get("/problems", response_model=schemas.ProblemListResponse)
def list_problems(
    ...
    return schemas.ProblemListResponse(
        items=[schemas.ProblemResponse.model_validate(p) for p in problems],
...
def get_problem(problem_id: int, db: Session = Depends(get_db)):
    ...
    return schemas.ProblemResponse.model_validate(problem)


@router.post("/problems", response_model=schemas.ProblemResponse, status_code=201)
